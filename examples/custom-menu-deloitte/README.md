# Custom Menu — Deloitte Example

This is a ready-to-use Canvus custom menu themed for Deloitte, including branded icons, sample content (images, PDFs, videos), and a fully configured `menu.yml`.
...
Extract the `custom-menu-deloitte.zip` into a `menu/` folder alongside your `mt-canvus.ini`.
...
Extract to:
```
%PROGRAMDATA%\MultiTaction\canvus\menu\custom-menu-deloitte\
```
...
Extract to:
```
~/MultiTaction/canvus/menu/custom-menu-deloitte/
```
...
```ini
[canvas]
custom-menu=menu/custom-menu-deloitte/menu.yml
```
...
>
> The path is **relative to the location of `mt-canvus.ini`**, so `menu/custom-menu-deloitte/menu.yml` works as long as you extracted the zip into the `menu/` folder as described above.
