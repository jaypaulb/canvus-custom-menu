# Custom Menu — KPMG Example

This is a ready-to-use Canvus custom menu themed for KPMG, including branded icons, sample content (images, PDFs, videos), and a fully configured `menu.yml`.
...
Extract the `custom-menu-kpmg.zip` into a `menu/` folder alongside your `mt-canvus.ini`.
...
Extract to:
```
%PROGRAMDATA%\MultiTaction\canvus\menu\custom-menu-kpmg\
```
...
Extract to:
```
~/MultiTaction/canvus/menu/custom-menu-kpmg/
```
...
```ini
[canvas]
custom-menu=menu/custom-menu-kpmg/menu.yml
```
...
>
> The path is **relative to the location of `mt-canvus.ini`**, so `menu/custom-menu-kpmg/menu.yml` works as long as you extracted the zip into the `menu/` folder as described above.
